...
func (userApi *UserApi) TokenNext(c *gin.Context, user database.User) {
	...
	//_, err = jwtService.GetRedisJWT(user.UUID)
	...
	if global.Config.System.UseMultipoint {
		...
		//}, "Successful login", c)
		//return
		if _, err := jwtService.GetRedisJWT(user.UUID); !errors.Is(err, redis.Nil) {
			response.FailWithMessage("Multipoint login is forbidden", c)
			return
		}
