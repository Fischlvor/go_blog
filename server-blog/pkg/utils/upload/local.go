...
func (*Local) UploadImageWithPrefix(file *multipart.FileHeader, pathPrefix string) (string, string, error) {
	size := float64(file.Size) / float64(1024*1024)
	if size >= float64(global.Config.Upload.Size) {
		return "", "", fmt.Errorf("the image size exceeds the set size, the current size is: %.2f MB, the set size is: %d MB", size, global.Config.Upload.Size)
	}
	...
	filename := utils.MD5V([]byte(name)) + "-" + time.Now().Format("20060102150405") + ext
	// 使用传入的路径前缀
	path := global.Config.Upload.Path + "/" + pathPrefix + "/"
	...
	// 返回相对路径（不带域名）
	return pathPrefix + "/" + filename, filename, nil
}
